...
class ChatMessage:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.tool_calls:
            result["tool_calls"] = self.tool_calls  # type: ignore
        return result
...
class OllamaAdapter:
    ...
    def show_model(self, name: str, verbose: bool = False) -> Dict[str, Any]:
        ...
        payload = {"model": name}
        if verbose:
            payload["verbose"] = verbose    # type: ignore
    ...
    def generate_embedding_legacy(
        ...
        if options:
            payload["options"] = options.to_dict()    # type: ignore
        if keep_alive:
            payload["keep_alive"] = keep_alive
    ...
    def load_model(self, model: str) -> Dict[str, Any]:
        ...
        return self.generate(model=model, prompt="", stream=False)      # type: ignore

    def unload_model(self, model: str) -> Dict[str, Any]:
        """Unload a model from memory"""
        return self.generate(model=model, prompt="", keep_alive="0", stream=False)  # type: ignore

    def load_model_chat(self, model: str) -> Dict[str, Any]:
        """Load a model into memory using chat endpoint""" 
        return self.chat(model=model, messages=[], stream=False)   # type: ignore

    def unload_model_chat(self, model: str) -> Dict[str, Any]:
        """Unload a model from memory using chat endpoint"""
        return self.chat(model=model, messages=[], keep_alive="0", stream=False)   # type: ignore
